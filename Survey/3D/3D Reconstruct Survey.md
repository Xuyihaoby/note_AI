...
Scaffold-GS 提出了一种基于锚点的分层、区域感知 3D 场景表示方法，从SfM得到的点云构建稀疏体素网格，将体素中心作为锚点，每个锚点配备局部上下文特征、缩放因子和可学习偏移量，并通过多分辨率特征融合与视图相关权重，生成锚点特征，提升视图适应性。在视图锥体内，每个可见锚点生成 k 个神经高斯，其位置由锚点位置与可学习偏移量、缩放因子计算得出；不透明度、颜色、旋转四元数、缩放等属性，则通过特定 MLP，基于锚点特征、相机与锚点的相对距离和视图方向实时预测，通过不透明度阈值过滤冗余高斯，保证渲染效率；生长操作通过将空间量化为多分辨率体素，对梯度超过阈值的体素添加新锚点，填补纹理缺失或观测不足区域；修剪操作则累计的不透明度，移除无法生成足够不透明度高斯的锚点。
...
7 维高斯表示可视为对 4DGS（空间–时间）与 6DGS（空间–视角）的统一扩展。每个场景元素被建模为一个联合的 7 维高斯分布，同时编码空间位置、时间和视角方向，并通过完整协方差显式刻画三者之间的相关性。在给定时间与视角条件下，该高斯可被条件化为一个 3D 高斯，从而无缝接入标准 3DGS 的渲染流水线，实现几何随时间演化及视角相关形变。高斯不透明度随时间与方向连续调制，支持动态出现与视角依赖贡献。结合时间编码与轻量级 MLP 的残差修正以及时空感知的高斯细化策略，该表示在保持 3DGS 高效性的同时，实现了对动态场景和视角相关效应的统一建模。
...
提出了一种紧凑型 3D Gaussian 表示方法，用于替代标准 3D Gaussian Splatting（3DGS）中“高质量但很占存储”的场景表示方式。作者的核心目标有两个：一是减少高斯数量，二是压缩每个高斯的属性表示。为此，他们设计了三项关键技术：基于体积和透明度的可学习高斯掩码来删除冗余高斯；用网格神经场替代每个高斯单独存储的球谐系数，以更紧凑地表示视角相关颜色；再用残差向量量化（R-VQ）码本来压缩几何属性（尺度与旋转）。


#### HAC: Hash-grid Assisted Context for 3D Gaussian Splatting Compression\HAC++: Towards 100X Compression of 3D Gaussian Splatting

HAC++在Scaffold-GS的锚点式高斯表示基础上，进一步从“锚点之间”和“锚点内部”两个维度建模冗余信息，以显著提升压缩性能。其核心由三个模块构成：哈希网格辅助上下文、锚点内部上下文模型以及自适应偏移掩码。首先，HAC利用锚点位置在哈希网格中插值得到的哈希特征作为上下文，借助空间结构的相关性对锚点属性的量化步长及其概率分布进行预测，从而减少锚点间冗余并降低算术编码所需的比特量。与此同时，锚点内部上下文模型进一步关注单个锚点内部特征的通道相关性，通过分块并基于已解码部分预测当前特征块的统计分布，显著压缩了锚点特征自身的冗余。两种上下文的预测结果最终通过高斯混合模型自适应融合。为进一步消除结构冗余，HAC++设计自适应偏移掩码，对每个高斯元自动判断有效性；无效高斯元不参与渲染与熵编码，若某锚点下的高斯元全部无效，则整个锚点可被裁剪，从源头减少存储开销。

在量化方面，HAC++采用训练加噪、推理取整的方式，同时通过由哈希特征预测的自适应量化步长应对不同属性的动态范围差异，以在保持保真度的同时实现更高效的熵建模。此外，哈希网格本身也被二值化并通过出现频率进行比特消耗估计，使其能以极低成本存储。整个训练过程分阶段逐步引入量化与上下文建模，以确保稳定性；编码阶段则依次对锚点位置、哈希网格和锚点属性进行解码，结合哈希特征及上下文模型完成逐块的算术解码。
...
将高斯基元投影回由粗解码得到的RGB、深度等特征图中，通过交叉注意力和MLP，计算球谐系数的残差，并与粗解码结果相加，从而恢复丰富的纹理细节。此过程还引入了位移特征以实现遮挡感知。
...
许多工作依赖 SDF 等场表示及等值面提取，这类方法难以稳健处理开放曲面、非流形结构和封闭内部几何；同时，几何与材质通常被分开建模，导致形状—外观对齐不足，且高分辨率生成常受限于潜变量 token 过多。作者据此认为，三维生成需要一种既能原生表达复杂拓扑与 PBR 材质、又足够紧凑的结构化潜表示。为此，本文提出以 O-Voxel 为核心的生成框架：首先用统一的稀疏体素结构编码几何与材质，再通过 Sparse Compression VAE 学习高压缩潜空间，最后在该潜空间中训练 flow matching 生成模型，实现高分辨率三维资产生成。

在表示层面，本文将三维资产写为 $f=\{(f_i^{\text{shape}},f_i^{\text{mat}},p_i)\}_{i=1}^{L}$，其中 $p_i$ 是激活体素位置，$f_i^{\text{shape}}$ 与 $f_i^{\text{mat}}$ 分别表示局部几何和材质特征。不同于基于场函数的方法，作者直接利用 mesh 与体素边的相交关系构造几何约束，并为每个 active voxel 定义 dual vertex、边交标记和三角化权重。dual vertex 通过最小化
$$
\min_{v \in \text{voxel}} e(v)=\sum_i d_{\Pi,i}^2+\lambda_{\text{bound}}\sum_j d_{L,j}^2+\lambda_{\text{reg}}d_{\hat q}^2
$$
得到，其中平面项约束局部表面位置，边界项增强开放曲面的表达，正则项提高稳定性。由此，本文在不依赖 watertight 假设的情况下实现了对复杂拓扑的统一建模。材质方面，每个体素存储 $f_i^{\text{mat}}=(c_i,m_i,r_i,\alpha_i)$，分别对应 base color、metallic、roughness 和 opacity。作者将 voxel center 投影到相交三角面上，在相应 UV 位置采样 PBR 贴图，并按点到表面的距离加权平均，从而获得局部材质；重建时，再通过邻近体素的三线性插值恢复连续表面属性，使几何与材质始终处于同一三维坐标系中。

为解决结构化表示压缩率不足的问题，本文进一步设计了 Sparse Compression VAE。其关键思想不是简单池化体素，而是在下采样时将局部 $2\times2\times2$ 子体素的特征重排到通道维，即
$$
F^{\text{raw}}_{\text{coarse}}=\text{stack}(F_{\text{child1}},\ldots,F_{\text{child8}}),
$$
随后再压缩为粗层特征。作者认为，这种 space-to-channel 的重排能比直接平均更好地保留局部结构，从而支持高倍率压缩。结合稀疏卷积、early-pruning 上采样和轻量残差块，SC-VAE 学得了适合高分辨率生成的紧凑潜空间。基于此，本文将生成过程分为稀疏结构生成、几何生成和材质生成三个阶段，并分别使用 DiT 形式的 flow matching 模型在潜空间中建模。由于几何与材质统一生成且潜变量高度紧凑，方法在保持复杂结构与真实材质的同时，也显著提升了高分辨率三维生成的效率与可扩展性。
...
沿用Tet-time Training的思想利用当前观测与记忆之间的交叉注意力分数，估计两者的匹配可靠性，并将该置信度直接作为状态更新的学习率。当观测与某些状态token高度一致时，对应状态将被更大幅度地更新；反之则更多保留历史信息。

<img src="https://note-ai-survey.oss-cn-hangzhou.aliyuncs.com/blog_imageimage-20260113000653606.png" alt="image-20260113000653606" style="zoom:50%;" />
...
Depth Anything 3 构建了统一的几何表示，以兼容单目、视频和多视角输入。模型以像素级射线作为核心几何描述，将深度与相机姿态的学习统一到同一表示中：每个像素对应的射线由相机中心和方向组成，使网络能够直接学习像素到三维空间的映射关系。姿态恢复不再直接预测旋转矩阵，而是通过射线与标准相机射线之间的几何变换推导出内参与外参；推理阶段则由轻量级相机头直接预测姿态以降低计算成本。网络以单个DINOv2 作为主干，通过交替进行视角内与跨视角注意力实现多视角融合。相机条件通过相机令牌注入，支持在有姿态与无姿态环境下的灵活推理。深度与射线由双 DPT 头解码，保持高分辨率空间结构。

训练采用教师学生范式：教师模型在大规模合成数据上训练，可提供高质量的相对深度；其输出通过鲁棒对齐流程与真实度量深度匹配后，用作学生模型的伪标签。学生模型在大规模无标签图像上学习这些几何一致的伪监督信号。损失函数同时约束深度、射线和重投影几何，并对尺度进行规范化以提升稳定性。

在下游应用中，通过加入专门的头部，模型可直接从图像预测像素级 3D 高斯参数，实现前馈式的新视角合成。进一步的姿态自适应设计允许模型同时处理带姿态与无姿态数据，并通过局部坐标系、高斯球谐颜色表示和冻结主干等策略提升真实场景鲁棒性。
...
现有 VGGT 在流式处理时若显式累积帧，会导致 KV 缓存随时间线性增长、内存不可控；而隐式压缩虽然节省内存，却会引入重建偏差。已有的 token pruning 方法通常依赖完整的 attention score 计算，这与 FlashAttention 的计算范式并不兼容。作者通过对 Query–Key 空间进行 PCA 可视化发现，仅基于 Key 的相似性度量即可达到与 attention-based pruning 相近的效果。该方法引入多样性感知的滚动内存机制：将首帧对应的 KV 缓存作为不可变的锚点长期保留，后续帧产生的令牌则作为可压缩的候选集合。压缩仅作用于候选令牌，并在每一层、每一个注意力头上独立执行，通过衡量候选 Key 在方向空间中的分散程度来刻画信息多样性，与整体分布越不相似的令牌被视为越重要，从而优先保留。在此基础上，进一步提出分层自适应的预算分配策略：根据各层候选令牌的平均多样性动态分配 KV 存储预算，使信息更丰富的层获得更多容量。
...
以DKM作为基础框架，将密集匹配划分为粗匹配阶段与逐级优化阶段。在粗匹配阶段，模型首先从两幅图像中提取鲁棒的全局特征，用于建立初始的跨图像对应关系；在优化阶段，则利用高分辨率、可精确定位的局部特征，对初始变形场进行逐尺度细化，最终得到亚像素级的匹配结果。在特征设计上，原始DKM假设单一编码器同时产生粗特征与精细特征，但这一假设在引入DINOv2后不再成立。DINOv2在大视角变化和外观变化下表现出极强的鲁棒性，适合用于全局匹配，但其固定步幅较大，缺乏精确定位能力。因此，将特征编码器解耦为两个部分：粗特征由冻结的DINOv2提取，以保证鲁棒性；精细特征则由卷积神经网络（如VGG19）提供，以支持后续的局部几何优化。

在粗匹配阶段，采用与DKM一致的高斯过程作为匹配编码器，用于建模跨图像特征间的相似性关系。本文引入了一种基于Transformer的匹配解码器，用分类而非直接回归的方式预测匹配结果目标图像的空间被离散为一组均匀分布的锚点，模型为每个源像素预测其落在各锚点上的概率分布，从而显式建模多模态匹配不确定性。最终，通过局部软解码的方式，将该分布转换为确定性的初始变形场。为了增强模型的泛化能力，Transformer解码器不使用任何位置编码，仅依赖特征相似性进行信息传播。在优化阶段，初始变形场通过一组多尺度卷积优化器逐步细化。每个尺度的优化器预测当前尺度下的残差位移以及对应的匹配置信度，并采用自粗到细的递归方式逐级更新变形场。由于在局部邻域内匹配分布通常为单峰，优化阶段可以被视为对条件分布均值的鲁棒回归。

在理论建模上，将不同尺度下的匹配分布视为由理想精确匹配分布经尺度相关的平滑操作得到，从而为粗到细的匹配过程提供了概率解释。在损失设计上，训练目标被统一为最小化预测分布与理论分布之间的KL散度：粗匹配阶段对应于锚点分类与匹配置信度的联合学习，而精细阶段则采用具有重尾特性的广义Charbonnier分布作为回归模型，以提升对离群匹配和初始化误差的鲁棒性。最终，总损失由粗匹配损失与各尺度精细优化损失组成，由于梯度在阶段间被显式切断，两部分可以独立优化，无需额外的权重平衡。

#### RoMa v2: Harder Better Faster Denser Feature Matching
将密集匹配任务拆解为粗匹配—精修两阶段流程。首先在较低分辨率上为每个像素预测近似匹配位置，再通过逐级精修实现亚像素级对齐，从而在鲁棒性与精度之间取得平衡。在粗匹配阶段，采用冻结的DINOv3作为特征编码器以增强泛化能力，并在 RoMa 框架基础上进行改进。不同于 RoMa 仅依赖高斯过程回归和单视图解码器的设计，本文引入多视图 Transformer 对两帧特征进行联合建模，通过交替的全局注意力与帧内注意力融合跨视角上下文信息。来自 DINOv3 不同层级的特征被拼接并投影至统一子空间后输入多视图 Transformer，输出的特征用于构建跨图像的相似度矩阵。基于该相似度分布，以注意力加权的方式聚合目标图像的位置信息，形成匹配嵌入，并与高层语义特征共同输入密集预测头，输出粗 warp 及其置信度。训练粗匹配器时，除回归 warp 的鲁棒损失与重叠度监督外，引入负对数似然损失，显式约束源图像中每个补丁在目标图像中的最佳匹配概率，从而增强匹配判别性并替代原有的分类式回归设计。

在精修阶段，冻结粗匹配器参数，仅对其输出的 warp 进行逐级优化。由于粗匹配已达到步长 4 的分辨率，精修仅在步长 {4, 2, 1} 上进行，相比 RoMa 和 DKM 显著降低了计算和内存开销。精修器沿用局部相关性与残差回归的基本思想，但针对局部相关计算实现了高效的 CUDA 内核。每一级精修器融合源图像特征、经 warp 对齐的目标图像特征、位移编码以及局部相关性信息，逐步细化匹配结果。此外，方法显式建模预测残差的不确定性，通过逐像素的高斯精度矩阵刻画匹配置信度，并以负对数似然形式进行监督；不同尺度下的精度预测具有可加性，从而实现分层不确定性建模。为提升稳定性，仅在可见且误差较小的区域训练该分支，并对残差进行梯度解耦。训练过程中还采用指数移动平均以减小系统性的亚像素偏差。
...
利用diffusion的涌现能力，不需要任何显式监督，仅提取反向过程中的特征就能进行语义的匹配。

#### MatchAnything: Universal Cross-Modality Image Matching with Large-Scale Pre-Training

该工作在数据构建层面进行设计，充分利用几何约束、多帧时序和跨模态一致性来生成高质量训练匹配。对于具有几何结构的多视图图像，方法通过深度图和相机参数将2D点反投影至3D空间并重投影到其他视角，结合投影深度误差与循环一致性误差对匹配进行严格筛选，从而获得可靠的真实对应关系。针对视频序列数据，利用匹配模型在相邻帧间生成局部对应，并通过轨迹合并与非极大值抑制构建长距离点轨迹，进一步借助多视图Transformer对轨迹进行亚像素级细化，从中筛选出具有充足共视关系的图像对作为伪真实匹配样本。除此之外，方法还通过随机采样单应性变换对单幅图像进行warp，生成具有已知对应关系的训练对，并将其与真实几何数据混合使用以提升多样性。在跨模态数据构建方面，采用像素对齐的图像生成模型将训练图像转换为不同模态，同时保持结构一致性，从而复用原有匹配标注；进一步引入单目深度估计结果作为训练模态，以显式引入更显著的结构变化，增强模型对真实跨模态差异的鲁棒性。整体而言，该数据构建策略通过融合几何约束、时序一致性与跨模态激励，为匹配模型提供了规模大、噪声可控且分布多样的监督信号。

#### UFM: A Simple Path towards Unified Dense Correspondence with Flow

这篇论文提出了 UFM（Unified Flow & Matching），目标是把传统上分开的两类稠密对应任务——光流估计和稠密匹配——统一到一个模型里。作者的核心观点是：这两类任务本质上都是“在两张图像之间寻找像素对应关系”，不应被割裂处理。为此，设计了一个简单的 Transformer 回归式架构，直接预测像素位移（即 flow）和共视性（covisibility）掩码，并在覆盖 12 个数据集的统一训练集上联合训练。实验表明，统一训练不仅可行，而且能让两个领域相互促进：UFM 同时在宽基线匹配和光流任务上超过多个专用方法；此外，作者还提出了一个新的宽基线评测基准 TA-WB，用于更系统地评估大视角变化下的稠密对应能力。

#### MapAnything: Universal Feed-Forward Metric 3D Reconstruction

提出了 MapAnything，目标是做一个“通用的、前馈式的、度量尺度的 3D 重建模型”：给定任意数量的输入图像，以及可选的几何信息（相机内参、位姿、深度、部分重建等），模型在一次前向传播中直接输出场景的度量尺度 3D 结构和相机信息。它的核心创新在于：不再像很多以往方法那样把几何、相机、尺度耦合在一个点图表示里，而是把多视图场景几何拆成一组射线方向、沿射线深度、相机位姿和全局尺度因子的“因子化表示”。这样一来，模型既能统一处理十多种 3D 视觉任务，也能自然接收异构输入，并在大量实验中达到或超过专用方法。
