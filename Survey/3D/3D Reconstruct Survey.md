...
需要注意的，GS的前提是需要进行colmap，从而得到相机的内外参数，以及对应的稀疏点云
...
1. 层次化高斯建模：将大场景划分为多个块，并在每个块内为所有高斯基元构建一棵层次化树（如BVH）。树中的叶节点是原始优化的高斯，中间节点则通过自底向上的方式对其子节点进行合并而来。合并过程依据父节点对整张图像的色彩贡献应等于其所有子节点贡献之和。。

2. 自适应渲染与平滑过渡：渲染时，根据视图和目标粒度，在树中自动选择一个最优的节点集合（称为一个 **Cut**）进行渲染。当视角变化导致所需细节层次改变时，系统通过在父节点与子节点属性（位置、颜色、旋转、缩放等）之间进行平滑插值

3. 联合优化与结构压缩：整个层次结构（包括中间节点）是可优化的。训练时，通过随机采样不同的目标粒度，迫使网络联合优化所有层次的表示，确保从粗糙到精细的每一层都能产生高质量的渲染结果。训练后，对树结构进*剪枝压缩，移除那些在多种粒度下都无需被选中的冗余中间节点。
...
2、确定对应的相机姿态下这个模块是否要参与训练

3、利用Level-of-Detail加速渲染，参照lightgaussian，不同的高度使用不同的压缩率
...
PoseDiffusion: Solving Pose Estimation via Diffusion-aided Bundle Adjustment（pose estimate）
